# Code Version: SRAuto20 - Skip header row "HBL" in extra mapping
...
with st.expander("품목, HS CODE 추가 (선택)", expanded=False):
    ...
    if extra_file:
        ...
        # 헤더 없이 A열→B열 매핑, 첫 행 'HBL'은 건너뛰기
        ex = pd.read_excel(extra_file, header=None)
        for _, row in ex.iterrows():
            hbl  = str(row[0]).strip()
            if hbl.upper() == 'HBL':  # 헤더 행 건너뛰기
                continue
...
if main_file:
    ...
    st.download_button("결과 메모장 다운로드", result,
